...
(deftest search-test
  (with-redefs [stub-core/fetch-all-contents (fn [] (->> fixtures (filter #(= (:type %) :db))))
                stub-core/embedding-text (fn [text] (->> fixtures
    ...
    (testing "search for building and clouds"
      (is (= (search "In which sample is there a description of a city's tall buildings touching the clouds?")
    ...
    (testing "search for break day and birds singing"
      (is (= (search "Which embedded text captures a scene at the break of day with birds singing?")
